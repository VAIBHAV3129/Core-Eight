...
export class Chip8 {
    ...
    fetch() {
        return (this.mem.read(this.pc) << 8) | this.mem.read(this.pc + 1);
    }
    ...
    cycle() {
        ...
        this.history.push({ cycle: this.cycles, pc: pcBefore, op: op, desc: desc });
    ...
    exec(op) {
        ...
        if ((op & 0xF000) === 0x1000) this.pc = nnn;
            ...
            const val = this.v[x];
            this.mem.write(this.i, Math.floor(val / 100));
            this.mem.write(this.i + 1, Math.floor((val % 100) / 10));
            this.mem.write(this.i + 2, val % 10);
        } else if ((op & 0xF0FF) === 0xF055) {
            for (let idx = 0; idx <= x; idx++) this.mem.write(this.i + idx, this.v[idx]);
            if (this.quirks.incrementI) this.i = (this.i + x + 1) & 0xFFFF;
        } else if ((op & 0xF0FF) === 0xF065) {
            for (let idx = 0; idx <= x; idx++) this.v[idx] = this.mem.read(this.i + idx);
            if (this.quirks.incrementI) this.i = (this.i + x + 1) & 0xFFFF;
        } else {
    ...
    draw(xReg, yReg, height) {
        ...
        for (let row = 0; row < height; row++) {
            const byte = this.mem.read(this.i + row);
    ...
    setKey(key, pressed) {
        ...
        if (this.halted) {
            ...
            if (wait === 'PRESS' && pressed) { this.halted = false; this.waitingForKey = null; }
            else if (wait === 'RELEASE' && !pressed) { this.halted = false; this.waitingForKey = null; }
            else if (wait === 'ANY_PRESS' && pressed) { this.halted = false; this.waitingForKey = null; }
            else if (wait === 'ANY_RELEASE' && !this.keys.some(k => k === 1)) { this.halted = false; this.waitingForKey = null; }
            else if (wait === 'V0_BLOCK' && pressed) { this.v[0] = key; this.halted = false; this.waitingForKey = null; }
            else if (wait === 'ANY_RELEASE_SCHIP' && !this.keys.some(k => k === 1)) { this.halted = false; this.waitingForKey = null; }
            else if (wait && typeof wait === 'object' && wait.type === 'SPECIFIC_RELEASE' && !pressed && key === wait.key) { this.halted = false; this.waitingForKey = null; }
        }
        if (this.waitingForKey !== null && typeof this.waitingForKey !== 'object' && !['PRESS', 'RELEASE', 'ANY_PRESS', 'ANY_RELEASE', 'ANY_RELEASE_SCHIP'].includes(this.waitingForKey)) {
            if (pressed) { this.v[this.waitingForKey] = key; this.waitingForKey = null; }
        }
    }
    ...
    testRunner(testCase) {
        ...
        for (let i = 0; i < testCase.cycles; i++) this.cycle();
        ...
        for (const [key, expected] of Object.entries(testCase.expected)) {
            ...
            if (actual !== expected) { results.passed = false; results.failures.push(`${key}: Exp ${expected}, Got ${actual}`); }
        }
        return results;
    }
    ...
    getOpcodeDetails(op) {
        ...
        const nn = op & 0x00FF;
        const nnn = op & 0x0FFF;
        return { binary: op.toString(2).padStart(16, '0'), masks: { x, y, n, nn, nnn }, desc: this.describe(op) };
    }
