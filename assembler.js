  export class Assembler {
    ...
    encode(text, line) {
      ...
      if (op === "DRW") {
        return 0xD000 | (this.reg(parts[1], line) << 8) | (this.reg(parts[2], line) << 4) | (this.num(parts[3], line) &
  0xF);
      }
    ...
    reg(val, line) {
      if (!this.isReg(val)) { this.errors.push(`Line ${line.line}: Invalid register "${val}". Expected V0-VF`); return
  0; }
